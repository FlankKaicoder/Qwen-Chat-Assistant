...
OUT_DIR="${1:-output/exp09_4_listen_kws_voice_photo_qa_manual}"
WAKE_TIMEOUT="${2:-25}"
CMD_SECONDS="${3:-5}"
RUN_TIMEOUT="${4:-160}"
...
echo "============================================================"
echo " Experiment 09.4: listen KWS -> voice photo QA"
echo "============================================================"
...
echo "本实验步骤："
echo "1) 运行后先说唤醒词：鲁班猫"
echo "2) 唤醒后进入命令录音，请说：看一下画面"
echo "3) 等待系统自动拍照、调用 Qwen3-VL，并听 TTS 播放"
echo
echo "注意："
echo "- 说完“鲁班猫”后停 1 秒，再说“看一下画面”。"
echo "- 第二句尽量说清楚，不要说太快。"
echo

echo "==================== 2. config key ===================="
grep -nE "mic_device|speaker_device|sample_rate|channels|input_channel|wake_input_gain|asr_input_gain|keywords_score|keywords_threshold|command_seconds|capture_script|photo_dir" \
...
if [ -n "$NEW_PHOTO_PATH" ] && [ -f "$NEW_PHOTO_PATH" ]; then
  ...
  echo "[WARN] no new voice photo found"
fi
echo

echo "==================== 6. stdout key lines ===================="
grep -nE "唤醒|wake|keyword|识别文本|正在调用|Qwen|将使用|整段|TTS|拍照|照片|画面|摄像头|图片|视觉|助手|回答|鲁班猫|Camera|photo|image" \
  "$STDOUT" || true
echo
...
# 提取 Qwen 回答。不同版本 stdout 顺序可能略有差异，所以这里采用保守方式：
# 从“正在调用 Qwen demo”之后开始，去掉提示行，保存剩余正文。
awk '
  BEGIN{flag=0}
...
if [ "$RC" = "0" ] \
  ...
  echo "[RESULT] Experiment 09.4 PASSED_LISTEN_KWS_VOICE_PHOTO_QA"
elif [ "$RC" = "124" ]; then
  echo "[RESULT] Experiment 09.4 TIMEOUT_OR_NO_WAKE"
elif [ "$RC" = "0" ] && [ "$NEW_PHOTO_COUNT" = "0" ]; then
  echo "[RESULT] Experiment 09.4 NEEDS_CHECK_NO_PHOTO_TRIGGER"
else
  echo "[RESULT] Experiment 09.4 FAILED_OR_NEEDS_CHECK"
fi
